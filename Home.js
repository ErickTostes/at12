...
const Home = ({ setIsAuthenticated }) => {
  ...
  useEffect(() => {
    ...
    const fetchFornecedores = async (token) => { 
  ...
  const filteredProducts = products.filter(product =>
    ...
    (selectedFornecedor ? product.fornecedor === selectedFornecedor : true)
  );
    ...
    <div className="home-container">
      ...
      <select value={selectedFornecedor} onChange={(e) => setSelectedFornecedor(e.target.value)}>
        ...
        {fornecedores.map((fornecedor) => (
          <option key={fornecedor._id} value={fornecedor.nome}>{fornecedor.nome}</option>
        ))}
      </select>
